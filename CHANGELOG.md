...
- **Caught up with upstream `apache/arrow-java`** (the fork had diverged 2026-01-27). Cherry-picked:
  - GH-994: `DatabaseMetaData` no longer NPEs when the server leaves SqlInfo values unpopulated.
  - GH-1004: fixed NPE in `ArrowFlightJdbcDriver#connect(url, Properties)`.
  - GH-1007: class loading no longer breaks when the direct-buffer allocator is unavailable (modern JDKs without `sun.misc.Unsafe` access).
  - GH-343: fixed `BaseVariableWidthVector`/`BaseLargeVariableWidthVector` offset-buffer serialization.
  - GH-1179: fixed var-width vector size when appending with a non-zero start offset.
  - GH-1116: fixed compressed-buffer prefix write and ZSTD `dstCapacity`.
  - Dependency updates: Avatica 1.26.0 -> 1.27.0, gRPC 1.78.0 -> 1.79.0, BouncyCastle 1.83 -> 1.84.

### Fixed
- **`TIMESTAMP WITH TIME ZONE` values are no longer skewed by the client's UTC offset.** A zoned Arrow vector carries an absolute instant, but `getTimestamp(Calendar)` adjusted the wall clock by `(calendarOffset − vectorOffset)` in the wrong direction and then re-interpreted it in the JVM default zone via `Timestamp.valueOf`, compounding to an error of up to **2× the client's UTC offset** (e.g. DBeaver in `America/New_York` showed `SELECT CURRENT_TIMESTAMP` = `15:24Z` as `19:24 -0400`, 8 hours ahead). `getTimestamp`/`getDate`/`getTime` on zoned vectors now return the exact epoch instant regardless of the supplied `Calendar` and the JVM default timezone — matching JDBC semantics and the behavior of major drivers (e.g. PostgreSQL). Naive (no-TZ) vectors keep their legacy wall-clock behavior. (Bug inherited from upstream `arrow-java`; upstream's own tests only asserted relative consistency, so it survived apache/arrow-java#464.)

### Testing
- New parameterized regression test (`testZonedVectorReturnsAbsoluteInstantRegardlessOfCalendarAndDefaultTz`) pins the DBeaver scenario: non-UTC JVM default TZ + client-timezone `Calendar` against UTC/Vancouver/Bangkok-zoned vectors must yield the exact epoch instant (fails 12/16 cases on the previous code). Existing with-calendar tests now assert zero shift for zoned vectors.

## [1.6.3] - 2026-06-02

### Fixed
- **`Connection.isValid()` now detects a stale server-side session when a `catalog` is configured, so connection pools recycle connections whose session has gone away.** GizmoSQL creates sessions lazily and applies the `catalog` (via `SetSessionOptions`/`USE`) exactly once, right after the Handshake. If a pooled connection's server-side session disappears — e.g. the underlying transport re-routes to a GizmoSQL replica that never saw it — the next query silently creates a fresh session in the default (`memory`) catalog and fails with a catalog error (`Table ... does not exist` / `expected catalog X but got memory`). Previously `isValid()` only checked TCP liveness (inherited from Avatica), so HikariCP and other pools handed out these stale connections. `ArrowFlightConnection.isValid(int)` now issues a lightweight `GetSessionOptions` probe (`ArrowFlightSqlClientHandler.isSessionValid(int)`) and reports the connection invalid when the session is gone, forcing a fresh Handshake + `SetSessionOptions` in the correct catalog. The probe runs **only when a catalog is configured** (there is otherwise no per-session catalog state to lose, and brand-new not-yet-used connections must not be discarded), and is **backwards compatible**: a server that does not implement `GetSessionOptions` (GizmoSQL older than v1.9.14, or non-GizmoSQL Flight SQL servers) answers `UNIMPLEMENTED`, which is treated as "cannot determine — keep the connection." Requires the cooperating GizmoSQL server change (`GetSessionOptions` no longer lazily creates sessions) to actually recycle evicted sessions; against servers without that change the probe is a no-op.

### Testing
- New unit tests in `ArrowFlightSqlClientHandlerTest` (`testIsSessionValid`) pin the probe's behaviour: live session → valid; evicted session (`UNAUTHENTICATED`) → invalid; unreachable (`UNAVAILABLE`) → invalid; `UNIMPLEMENTED` (older server) → valid.
- New integration test in `ConnectionTest` (`testIsValidWithCatalogToleratesServerWithoutGetSessionOptions`) connects with a `catalog` configured against a mock server that returns `UNIMPLEMENTED` for `GetSessionOptions` and asserts `isValid()` still returns `true` — end-to-end backwards-compatibility guard for pre-v1.9.14 servers.
