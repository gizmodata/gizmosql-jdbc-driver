...
public class MemoryUtil {
  ...
  static {
    try {
      ...
      try {
        ...
        maybeOffset = -1;
        logger.debug(
            "Cannot access the address field of java.nio.Buffer."
                + " DirectBuffer operations wont be available",
            e);
      }
  ...
  public static long getByteBufferAddress(ByteBuffer buf) {
    ...
    throw new UnsupportedOperationException(
        "Byte buffer address cannot be obtained because sun.misc.Unsafe"
            + " or java.nio.DirectByteBuffer.<init>(long, int) is not available");
  }
